...
/* raw-PCM mode: the file is pre-rendered signed 8-bit mono at MUS_RATE
 * (decoded from MP3 at package time) — playback is a looping buffer copy,
 * so there is no synthesis cost at all */
static int raw_mode;
static uint32_t raw_pos;
...
int mus_load(const char *path)
{
    ...
    if (memcmp(vgm, "Vgm ", 4) != 0) {
        /* not a VGM: treat the whole file as raw 8-bit PCM at MUS_RATE */
        raw_mode = 1;
        raw_pos = 0;
        music_ok = 1;
        return 1;
...
void mus_render(signed char *out, int n)
{
    ...
    if (raw_mode) {
        int done = 0;
        while (done < n) {
            uint32_t chunk = vgm_len - raw_pos;
            if (chunk > (uint32_t)(n - done))
                chunk = (uint32_t)(n - done);
            memcpy(out + done, vgm + raw_pos, chunk);
            raw_pos += chunk;
            done += (int)chunk;
            if (raw_pos >= vgm_len)
                raw_pos = 0;            /* loop */
